...
En fait, un user peut liker un ou plusieurs produits donc ia cardinalité du côté USER est (1, N), également pour celle de l'autre côté du produit, 
un produit peut être liker une ou plusierus fois (1, N).

Toutes les autres entités (ADDRESS, ORDER) ainsi que les cardinalités entre ces entités sont correctes.

N.B: Il existe plusieurs outils pour la réalisation des modèles conceptuels de données ou la modélisation de bases de données comme: 

     AnalyseSI, MySql Workbench, GitMind, ChartEdra, Draw.io ...
